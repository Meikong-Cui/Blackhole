...
public:
  PostProcessPass(const std::string &fragShader) {
    this->program = createShaderProgram("simple.vert", fragShader);
...
int main(int, char **) {
  ...
  // Main loop
  PostProcessPass passthrough("passthrough.frag");
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    static GLuint galaxy = loadCubemap("skybox_nebula_dark");
    static GLuint colorMap = loadTexture2D("color_map.png");
    static GLuint uvChecker = loadTexture2D("uv_checker.png");

    static GLuint texBlackhole = createColorTexture(SCR_WIDTH, SCR_HEIGHT);
    {
      RenderToTextureInfo rtti;
      rtti.fragShader = "blackhole_main.frag";
    ...
    {
      RenderToTextureInfo rtti;
      rtti.fragShader = "bloom_brightness_pass.frag";
    ...
    for (int level = 0; level < bloomIterations; level++) {
      RenderToTextureInfo rtti;
      rtti.fragShader = "bloom_downsample.frag";
    ...
    for (int level = bloomIterations - 1; level >= 0; level--) {
      RenderToTextureInfo rtti;
      rtti.fragShader = "bloom_upsample.frag";
    ...
    {
      RenderToTextureInfo rtti;
      rtti.fragShader = "bloom_composite.frag";
      rtti.textureUniforms["texture0"] = texBlackhole;
    ...
    {
      RenderToTextureInfo rtti;
      rtti.fragShader = "tonemapping.frag";
